...
def armado_de_cabezales(opcion_seleccionada, entrada_cantidad, lista_acciones, subtitulo, tabla_chapa):
    ...
    try:
        ...
        suficientes_piezas = all(verificar_suficientes_piezas(cursor, pieza, tipo_material, cantidad_eliminar) for pieza in piezas)

        if not suficientes_piezas:
            lista_acciones.insert(0, "No hay suficientes piezas en el stock para armar el cabezal.")
            return
...
def enviar_piezas_a_pulido(pieza, cantidad, tabla, tree, lista_acciones,):
    ...
    try:
        ...
        cursor.execute(f"UPDATE piezas_del_fundicion SET cantidad = cantidad = ? WHERE piezas = ?", (nueva_cantidad_stock_bruto, pieza_seleccionada))
...
def mostrar_datos_torno(arbol, tabla ):
    conn = sqlite3.connect("basedatospiezas.db")
    cursor = conn.cursor()
    cursor.execute(f"SELECT piezas, cantidad FROM {tabla} WHERE mecanizado = 'torno' AND piezas NOT IN ('caja_torneado_330', 'caja_torneado_300', 'caja_torneado_250')")
    datos = cursor.fetchall()
    conn.close()
    for item in arbol.get_children():
        arbol.delete(item)
...
def mostrar_datos_(arbol):
    conn = sqlite3.connect("basedatospiezas.db")
    cursor = conn.cursor()
    cursor.execute(f"SELECT piezas, cantidad FROM piezas_del_fundicion WHERE piezas IN ('caja_torneado_330', 'caja_torneado_300', 'caja_torneado_250')")
    datos = cursor.fetchall()
    conn.close()
    for item in arbol.get_children():
        arbol.delete(item)
...
def actualizar_pieza_torno(lista_predefinida, entrada_cantidad, res, table, tree):
    ...
    if entrada_actualizar.strip().isdigit():
        ...
        if entrada_actualizar < 0:
            ...
            conn = sqlite3.connect("basedatospiezas.db")
            cursor = conn.cursor()
            cursor.execute(f"SELECT cantidad FROM piezas_del_fundicion WHERE piezas=?", (actualizar_pieza,))
            cantidad_actual = cursor.fetchone()
            ...
            if cantidad_actual is not None:
                ...
                if cantidad_actual >= entrada_actualizar:
                    ...
                    cursor.execute(f"UPDATE piezas_del_fundicion SET cantidad=? WHERE piezas=?", (nueva_cantidad, actualizar_pieza))
                    conn.commit()

                    # Extrae el valor del Entry antes de pasarlo a la consulta SQL
                    cantidad_a_actualizar = entrada_cantidad.get()
                    cursor.execute("UPDATE piezas_finales_defenitivas SET cantidad = cantidad + ? WHERE piezas = ?", (cantidad_a_actualizar, actualizar_pieza))
                    conn.commit()
                    conn.close()
                    mostrar_datos(tree, table)  # Llama a la función para mostrar los datos actualizados
                    res.insert(0, f"Carga exitosa: Usted cargó {entrada_actualizar} {actualizar_pieza}:")
            ...
            if entrada_actualizar < 0:
                ...
                if cantidad_actual is not None:
                    ...
                    if cantidad_actual >= entrada_actualizar:
                        ...
                        conn.commit()
                        cantidad_a_actualizar = entrada_cantidad.get()
                        cursor.execute("UPDATE torno SET cantidad = cantidad + ? WHERE piezas = ?", (cantidad_a_actualizar, actualizar_pieza))
                        conn.commit()
                        cursor.execute("UPDATE piezas_finales_defenitivas SET cantidad = cantidad + ? WHERE piezas = ?", (cantidad_a_actualizar, actualizar_pieza))
                        conn.commit()
                        conn.close()
...
def mostrar_piezas_torno_terminado(arbol):
    conn = sqlite3.connect("basedatospiezas.db")
    cursor = conn.cursor()
    cursor.execute("SELECT piezas, cantidad FROM piezas_finales_defenitivas WHERE piezas IN ( 'manchon', 'manchon_250', 'eje_250', 'eje', 'rueditas', 'tornillo_guia', 'carros', 'movientos', 'carros_250');")
    datos = cursor.fetchall()
    conn.close()
    for item in arbol.get_children():
        arbol.delete(item)
...
def pulido_cabezal(entrada_agregar_porta_eje, lista_acciones):
    ...
    if entrada_actualizar.strip().isdigit():
        ...
        if entrada_actualizar < 0:
            ...
            try:
                cursor.execute("SELECT cantidad FROM piezas_del_fundicion WHERE piezas = 'cabezal_l'")
                cantidad_actual = cursor.fetchone()
                ...
                if cantidad_actual is not None:
                    ...
                    cursor.execute("UPDATE piezas_del_fundicion SET cantidad= cantidad  - ? WHERE piezas = 'cabezal_l'", (entrada_actualizar,))
                    conn.commit()
                    cursor.execute("UPDATE piezas_finales_defenitivas SET cantidad= cantidad + ? WHERE piezas = 'cabezal_l'", (entrada_actualizar,))
        ...
        if cantidad < 0:
            ...
            try:
                if pieza == "caja_torneado_330":
                    cursor.execute("SELECT cantidad FROM piezas_del_fundicion WHERE piezas = 'caja_330'")
                    cantidad_actual  = cursor.fetchone()
                    ...
                    if cantidad_actual is not None:
                        ...
                        if cantidad_actual >= cantidad:
                            ...
                            cursor.execute(f"UPDATE piezas_del_fundicion SET cantidad = ? WHERE piezas = 'caja_330'", (nueva_cantidad_enbruto,))

                            # Actualiza la cantidad de cajas tornadas
                            cursor.execute(f"UPDATE piezas_del_fundicion SET cantidad = cantidad + ? WHERE piezas = 'caja_torneado_330'", (cantidad,))
                        else:
                            lista_acciones.insert(0, f"No hay suficientes piezas de {pieza} en bruto disponibles.")    
                    else:
                    ...
                    cursor.execute("SELECT cantidad FROM piezas_del_fundicion WHERE piezas = 'caja_300'")
                    cantidad_actual  = cursor.fetchone()
                    ...
                    if cantidad_actual is not None:
                        ...
                        if cantidad_actual >= cantidad:
                            ...
                            # Actualiza la cantidad de cajas en bruto
                            cursor.execute(f"UPDATE piezas_del_fundicion SET cantidad = ? WHERE piezas = 'caja_300'", (nueva_cantidad_enbruto,))

                            # Actualiza la cantidad de cajas tornadas
                            cursor.execute(f"UPDATE piezas_del_fundicion SET cantidad = cantidad + ? WHERE piezas = 'caja_torneado_300'", (cantidad,))
                        else:
                            lista_acciones.insert(0, f"No hay suficientes piezas de {pieza} en bruto disponibles.")    
                    else:
                        lista_acciones.insert(0, f"No se encontró la pieza {pieza}.")    
                    ...
                    cursor.execute("SELECT cantidad FROM piezas_del_fundicion WHERE piezas = 'caja_250'")
                    cantidad_actual  = cursor.fetchone()
                    ...
                    if cantidad_actual is not None:
                        ...
                        if cantidad_actual >= cantidad:
                            ...
                            # Actualiza la cantidad de cajas en bruto
                            cursor.execute(f"UPDATE piezas_del_fundicion SET cantidad = ? WHERE piezas = 'caja_250'", (nueva_cantidad_enbruto,))

                            # Actualiza la cantidad de cajas tornadas
                            cursor.execute(f"UPDATE piezas_del_fundicion SET cantidad = cantidad + ? WHERE piezas = 'caja_torneado_250'", (cantidad,))
                        else:
                            lista_acciones.insert(0, f"No hay suficientes piezas de {pieza} en bruto disponibles.")    
                    else:
                        lista_acciones.insert(0, f"No se encontró la pieza {pieza}.")
...
def mostrar_cajas_bruto(arbol):
    conn = sqlite3.connect("basedatospiezas.db")
    cursor = conn.cursor()
    cursor.execute(f"SELECT piezas, cantidad FROM piezas_del_fundicion WHERE piezas IN ('caja_330', 'caja_300', 'caja_250')")
    datos = cursor.fetchall()
    conn.close()
    for item in arbol.get_children():
        arbol.delete(item)
